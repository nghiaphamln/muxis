...
async fn stress_test_many_keys_all_slots() {
    ...
    println!(
        "Total time: {:?}",
        set_duration + get_duration + del_duration
    );
}
...
async fn stress_test_hash_tag_batching() {
    ...
    println!(
        "Testing {} batches with {} keys each...",
        num_batches, keys_per_batch
    );
...
async fn stress_test_sustained_load() {
    ...
    println!(
        "Running sustained load test for {} seconds with {} workers...",
    ...
    println!(
        "  Ops/sec: {:.2}",
        ops as f64 / actual_duration.as_secs_f64()
    );
    println!(
        "  Ops/sec/worker: {:.2}",
        ops as f64 / actual_duration.as_secs_f64() / num_workers as f64
    );
}
